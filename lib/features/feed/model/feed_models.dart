class FeedPost {
  ...
  static FeedPost fromApi(Map<String, dynamic> post) {
    ...
    if (u is Map) {
      ...
      avatarUrl = _cleanUrl(a);
    } else {
      userName = (post['username'] ?? post['user_name'] ?? 'User').toString();

      final a = post['avatar'] ?? post['avatar_url'] ?? post['photo'];
      avatarUrl = _cleanUrl(a);
    }
    ...
    final images = <String>[];
    final rawImages = post['images'] ?? post['media'] ?? post['photos'];
...
class FeedComment {
  ...
  static FeedComment fromApi(Map<String, dynamic> raw) {
    ...
    if (u is Map) {
      userName = (u['username'] ?? u['name'] ?? 'User').toString();
      avatarUrl = FeedPost._cleanUrl(
        u['avatar'] ?? u['avatar_url'] ?? u['photo'],
      );
    } else {
      userName = (raw['username'] ?? raw['user_name'] ?? 'User').toString();
      avatarUrl = FeedPost._cleanUrl(
        raw['avatar'] ?? raw['avatar_url'] ?? raw['photo'],
      );
    }
